...
abstract class TweetSet extends TweetSetInterface:
   ...
   * and be implemented in the subclasses?
   */
  def union(that: TweetSet): TweetSet = that.unionAcc(this.unionAcc(Empty()))
...
object GoogleVsApple:
  ...
  def containsAny(keywords: List[String])(t: Tweet): Boolean =
    keywords.map(t.text.contains).exists(identity) //map = new collection after aplying a condition, identity returns it's input value

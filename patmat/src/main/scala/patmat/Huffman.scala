...
trait Huffman extends HuffmanInterface:
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    ...
    makeLeafList(freqs.sortBy(x => x._2))
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeAcc(sub_tree: CodeTree, code: List[Bit]): List[Char] ={
      sub_tree match{
        case Leaf(char,_) => char :: decodeAcc(tree,code)
        case Fork(left,right,_,_) =>{
          if(code.isEmpty){
            ...
            code.head match{
              case 0 => decodeAcc(left,code.tail)
              case 1 => decodeAcc(right,code.tail)
            }
          }
        }
      }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table.head match
    case (character,code) =>
      if(character == char) code else codeBits(table.tail)(char)
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    val code = codeBits(table)_

    def quickEncoder(characters: List[Char]):List[Bit] = characters match
      case Nil => List[Bit]()
      case x::xs => code(x) ::: quickEncoder(xs)
